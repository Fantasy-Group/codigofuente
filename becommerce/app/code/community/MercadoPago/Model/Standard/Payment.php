...
class MercadoPago_Model_Standard_Payment extends Mage_Payment_Model_Method_Abstract{
	...
	function makePreference(){
		...
		foreach ($order->getAllVisibleItems() as $item) {
			...
			// codigo bekommerce init here
			$baseCurrencyCode = Mage::app()->getStore()->getBaseCurrencyCode();
			$currentCurrencyCode = Mage::app()->getStore()->getCurrentCurrencyCode();
			$price_total = $produto->getFinalPrice();
			$price_converted = Mage::helper('directory')->currencyConvert($price_total,$baseCurrencyCode,$currentCurrencyCode);
			// codigo bekommerce ends here
			//get image
			try{
			...
			$arr['items'][] = array(
				...
				"unit_price" => (float) number_format($price_converted, 2, '.', '')
			);
