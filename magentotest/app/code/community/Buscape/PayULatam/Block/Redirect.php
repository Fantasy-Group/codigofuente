...
class Buscape_PayULatam_Block_Redirect extends Mage_Core_Block_Abstract
{
    
    protected function _toHtml()
    {
        $standard = Mage::getModel('payulatam/'.$this->getRequest()->getParam("type"));
                
        $form = new Varien_Data_Form();
        
        $form->setAction($standard->getPayULatamUrl())
            ->setId('payulatam_payment_checkout')
            ->setName('payulatam_payment_checkout')
            ->setMethod('POST')
        ...
        $html .= ''.$this->__('Aguarde ... você será redirecionado para a PayU Latam em <span id="tempo">5</span> segundos.</font>');
        $html .= '<div>';
        $html .= $form->toHtml();
        $html .= '<script type="text/javascript">
                    function setTempo(){
                        ...
                        if (tempo - 1 < 0){
                            document.getElementById("payulatam_payment_checkout").submit();
                        }else{
